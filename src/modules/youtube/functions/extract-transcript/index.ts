 ...
 */
function validateVideoId(value: unknown): { isValid: boolean; errors: string[] } {
 ...
 */
function validateLanguageCode(value: unknown): { isValid: boolean; errors: string[] } {
  if (typeof value !== 'string') {
    return { isValid: false, errors: ['Must be a string'] };
  }
...
function validateRequest(data: any): { isValid: boolean; errors: string[] } {
  const result = validateRequestBody<ExtractTranscriptRequest>(data, transcriptRequestSchema);
  return {
 ...
 */
async function saveTranscript(supabase: any, videoId: string, transcript: any): Promise<void> {
  try {
 ...
 */
async function fetchTranscript(videoId: string, languageCode: string): Promise<any> {
  ...
  try {
    ...
    if (error instanceof Error) {
      if (error.message.includes('No transcripts available')) {
        return createCorsErrorResponse('No transcripts available for this video', 404, requestId, {
          code: 'NO_TRANSCRIPTS',
        });
      }

      if (error.message.includes('Failed to fetch video page')) {
        return createCorsErrorResponse('Video not found or is private', 404, requestId, {
          code: 'VIDEO_NOT_FOUND',
        });
      }

      if (error.message.includes('Failed to fetch transcript')) {
        return createCorsErrorResponse('Failed to fetch transcript from YouTube', 502, requestId, {
