// Checkout session endpoint using Layer 1 & 2 utilities

import { createCorsErrorResponse, createCorsResponse, createCorsSuccessResponse } from '@/cors';
import { AppError, handleUnknownError } from '@/shared-errors';
    ...
    try {
      ...
      const session = await this.stripe.checkout.sessions.create({
        ...
        allow_promotion_codes:
          request.allowPromotionCodes !== undefined ? request.allowPromotionCodes : true,
...
serve(async req => {
  ...
  try {
    ...
    if (!userInfo) {
      return createCorsErrorResponse('Valid authentication token required', 401, requestId, {
    ...
    if (!validation.isValid) {
      return createCorsErrorResponse('Invalid request parameters', 400, requestId, {
