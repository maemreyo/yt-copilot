...
import { z } from 'zod';
import { type AnalyzeContentResponse } from '../types';
...
const rateLimiterFree = createRateLimiter({
  ...
  maxRequests: parseInt(denoEnv.get('ANALYSIS_RATE_LIMIT_FREE') || '3'),
...
const rateLimiterPremium = createRateLimiter({
  windowMs: 24 * 60 * 60 * 1000, // 24 hours
  maxRequests: parseInt(denoEnv.get('ANALYSIS_RATE_LIMIT_PREMIUM') || '20'),
  keyGenerator: request => {
    const auth = request.headers.get('authorization');
    return auth ? `analyze:${auth}` : `analyze:${request.headers.get('x-real-ip')}`;
  },
...
serve(
  async request => {
          ...
          error: {
            ...
            message: isPremium
              ? `Premium rate limit exceeded: ${denoEnv.get(
                  'ANALYSIS_RATE_LIMIT_PREMIUM'
                )} analyses per day`
              : `Free rate limit exceeded: ${denoEnv.get(
                  'ANALYSIS_RATE_LIMIT_FREE'
    ...
    try {
      // Check cache first
      const cachedAnalysis = await aiCacheManager.getCachedAnalysis(video_id, analysis_type);
      ...
      if (segments && segments.length > 0) {
        ...
        if (selectedSegments.length === 0) {
          throw createAppError(ErrorType.VALIDATION_ERROR, 'Invalid segment indices provided', {
            segments,
      ...
      transcriptText = selectedSegments.map((segment: any) => segment.text).join(' ');
      ...
      if (analysis_type === 'fact_opinion' && isPremium) {
        ...
        if (mainOpinions) {
          const { suggestions: counterSuggestions } =
            await openAIClient.generateCounterPerspectives(mainTopic, mainOpinions);
