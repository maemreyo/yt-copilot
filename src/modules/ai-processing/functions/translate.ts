...
const rateLimiterFree = createRateLimiter({
  ...
  maxRequests: parseInt(denoEnv.get('TRANSLATION_RATE_LIMIT_FREE') || '100'),
...
const rateLimiterPremium = createRateLimiter({
  windowMs: 60 * 60 * 1000, // 1 hour
  maxRequests: parseInt(denoEnv.get('TRANSLATION_RATE_LIMIT_PREMIUM') || '500'),
  keyGenerator: request => {
    const auth = request.headers.get('authorization');
    return auth ? `translate:${auth}` : `translate:${request.headers.get('x-real-ip')}`;
  },
  ...
  async request => {
          ...
          error: {
            ...
            message: isPremium
              ? `Premium rate limit exceeded: ${denoEnv.get(
                  'TRANSLATION_RATE_LIMIT_PREMIUM'
                )} translations per hour`
              : `Free rate limit exceeded: ${denoEnv.get(
                  'TRANSLATION_RATE_LIMIT_FREE'
                )} translations per hour`,
          },
        }),
        {
    ...
    try {
      ...
      if (cachedTranslation) {
        ...
        const definitions = isSingleWord ? await wordsAPIClient.getDefinitions(text) : undefined;
      ...
      // Perform translation
      const translationResult = await translationClient.translate(text, target_lang, source_lang);

      // Get additional data for single words
      const isSingleWord = text.split(' ').length === 1;
      const definitions = isSingleWord ? await wordsAPIClient.getDefinitions(text) : undefined;
      const pronunciation = isSingleWord ? await wordsAPIClient.getPronunciation(text) : undefined;
